...
| `robogame_adapters` | 实机相机后端：`CameraMarkerDetector`（AprilTag）、`CameraBlockDetector`（HSV）、图像转换（numpy，不用 cv_bridge） |
...
启动流程满足完全自主要求：上电 → systemd 拉起 → SELF_CHECK →
等待物理启动按钮（START_EVENT，走 STM32 GPIO，不依赖网络）→ 自主比赛。

## 赛前检查清单

- [ ] `make check-config` 全绿（无 TODO 残留、路线图连通）
- [ ] `python scripts/gen_protocol_header.py --check` 通过（协议未漂移）
- [ ] real_field.yaml 所有数值为本场实测值
...
| 现象 | 排查 |
|---|---|
| mission_node 起不来，ImportError core | 没装核心包：`pip install -e .`（在仓库根目录） |
| MCU 心跳超时 → SAFE_STOP | 检查 `/dev/serial0`、波特率、STM32 固件是否包含最新协议头 |
| 相机无检测 | 确认话题名与 launch 参数一致；`v4l2-ctl --list-devices` 检查设备 |
| 定位置信度持续走低 | marker 没识别到：检查 tag 打印质量、`localization_camera` 内参与外参 |
| 巡线丢失（LINE_STATE=lost） | 线传感器标定 / 光照；必要时下发新的 `CMD_LINE_FOLLOW_CONFIG` |
